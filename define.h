...
#define NW_BITS         log2(NUM_WARPS)

#define NT_BITS         log2(NUM_THREADS)

#define NC_BITS         log2(NUM_CORES)

#define NB_BITS         log2(NUM_BARRIERS)

#define NUM_IREGS       32

#define NRI_BITS        log2(NUM_IREGS)

#define NTEX_BITS       log2(NUM_TEX_UNITS)
...
#define ICACHE_MEM_ADDR_WIDTH   (32 - log2(ICACHE_LINE_SIZE))

// Memory request tag bits
#define ICACHE_MEM_TAG_WIDTH    log2(ICACHE_MSHR_SIZE)
...
// Core request tag bits
#define LSUQ_ADDR_BITS          log2(LSUQ_SIZE)
...
// Memory request address bits
#define DCACHE_MEM_ADDR_WIDTH   (32 - log2(DCACHE_LINE_SIZE))
...
// Memory request tag bits
#define _DMEM_ADDR_RATIO_W      log2(DCACHE_LINE_SIZE / DCACHE_WORD_SIZE)
#define _DNC_MEM_TAG_WIDTH      (log2(DCACHE_NUM_REQS) + _DMEM_ADDR_RATIO_W + DCACHE_CORE_TAG_WIDTH)
#define DCACHE_MEM_TAG_WIDTH    MAX((log2(DCACHE_NUM_BANKS) + log2(DCACHE_MSHR_SIZE) + NC_TAG_BIT), _DNC_MEM_TAG_WIDTH)

// Merged D-cache/I-cache memory tag
#define L1_MEM_TAG_WIDTH        (MAX(ICACHE_MEM_TAG_WIDTH, DCACHE_MEM_TAG_WIDTH) + log2(2))
...
// bank address offset
#define SMEM_BANK_ADDR_OFFSET   log2(STACK_SIZE / SMEM_WORD_SIZE)
...
#define L2_CORE_TAG_WIDTH        (DCACHE_CORE_TAG_WIDTH + log2(NUM_CORES))

// Memory request data bits
#define L2_MEM_DATA_WIDTH        (L2_CACHE_LINE_SIZE * 8)

// Memory request address bits
#define L2_MEM_ADDR_WIDTH        (32 - log2(L2_CACHE_LINE_SIZE))
...
// Memory request tag bits
#define _L2_MEM_ADDR_RATIO_W     log2(L2_CACHE_LINE_SIZE / L2_WORD_SIZE)
#define _L2_NC_MEM_TAG_WIDTH     (log2(L2_NUM_REQS) + _L2_MEM_ADDR_RATIO_W + L1_MEM_TAG_WIDTH)
#define _L2_MEM_TAG_WIDTH        MAX((log2(L2_NUM_BANKS) + log2(L2_MSHR_SIZE) + NC_TAG_BIT), _L2_NC_MEM_TAG_WIDTH)
#define L2_MEM_TAG_WIDTH         ((L2_ENABLE) ? _L2_MEM_TAG_WIDTH : (L1_MEM_TAG_WIDTH + log2(L2_NUM_REQS)))
...
// Input request tag bits
#define L3_CORE_TAG_WIDTH        (L2_CORE_TAG_WIDTH + log2(NUM_CLUSTERS))

// Memory request data bits
#define L3_MEM_DATA_WIDTH        (L3_CACHE_LINE_SIZE * 8)

// Memory request address bits
#define L3_MEM_ADDR_WIDTH        (32 - log2(L3_CACHE_LINE_SIZE))
...
// Memory request tag bits
#define _L3_MEM_ADDR_RATIO_W     log2(L3_CACHE_LINE_SIZE / L3_WORD_SIZE)
#define _L3_NC_MEM_TAG_WIDTH     (log2(L3_NUM_REQS) + _L3_MEM_ADDR_RATIO_W + L2_MEM_TAG_WIDTH)
#define _L3_MEM_TAG_WIDTH        MAX((log2(L3_NUM_BANKS) + log2(L3_MSHR_SIZE) + NC_TAG_BIT), _L3_NC_MEM_TAG_WIDTH)
#define L3_MEM_TAG_WIDTH         ((L3_ENABLE) ? _L3_MEM_TAG_WIDTH : (L2_MEM_TAG_WIDTH + log2(L3_NUM_REQS)))
...
#define VX_CSR_ID_WIDTH         log2(NUM_CLUSTERS * NUM_CORES)

#define  TO_FULL_ADDR(x) concatenate ( x ,  (unsigned int)log2(number)+1, 0 ,  32-((unsigned int)log2(number)+1)) ;
